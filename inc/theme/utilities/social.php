...
function wputh_get_social_links_html($wrapper_classname = 'header__social', $display_type = false, $template = '', $wpu_social_links = array()) {
    ...
    foreach ($wpu_social_links as $id => $link) {
        ...
        case 'custom':
            ...
            break;
        case 'icon':
            $icon_classname = apply_filters('wputheme_social_links_icon_classname', 'icon icon_' . $id, $id, $link);
            $html .= '<i aria-hidden="true" class="' . esc_attr($icon_classname) . '"><span class="screen-reader-text">' . $link['name'] . '</span></i>';
            break;
        default:
            $html .= $link['name'];
        }
